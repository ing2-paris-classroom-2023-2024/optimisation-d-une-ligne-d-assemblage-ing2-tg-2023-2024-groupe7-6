//
// Created by Hugo on 27/11/2023.
//
#include "projhugo.h"

int *creation_tab_op_reelles(char *nom_fichier,t_graphe* graphe1){
    FILE *fp=fopen(nom_fichier,"r");
    graphe1->tab_operations_reelles = malloc((graphe1->num_operation_maxi)*sizeof(int));

    for(int i=0;i<graphe1->num_operation_maxi;i++){
        graphe1->tab_operations_reelles[i] = -1;
    }

    if(fp==NULL){
        printf("Erreur d ouverture du fichier d'operations reelles.\n");
        exit(1);
    }
    ...
    while(fscanf(fp,"%d%f",&op,&temps)==2){
        graphe1->tab_operations_reelles[op]=1;
    }
    return graphe1->tab_operations_reelles;
}


void afficher_matrice_exclusions(t_graphe *graphe1){
    for(int i=0;i<(graphe1->num_operation_maxi + 1);i++){
        for(int j=0;j<(graphe1->num_operation_maxi + 1);j++){
            printf("[%d]",graphe1->matrice_exclusions[i][j]);
        }
        printf("\n");
    }
}

int num_operation_max(char* file){
    FILE *fp=fopen(file, "r");

    if(fp==NULL){
        printf("Erreur d ouverture du fichier operation_max.\n");
        exit(1);
    }
...
t_graphe* gestion_donnees_graphe(int nb_operations,int op_max,char *file_name){
    t_graphe *graphe1=malloc(sizeof(t_graphe));
    ...
    graphe1->ordre_sous_graphe=0;
    graphe1->num_operation_maxi=op_max;
    graphe1->tab_stations=malloc((op_max+1)*sizeof(t_station*));
    graphe1->nb_stations=0;
    graphe1->nb_paires_exclusion = 0;
    graphe1->matrice_exclusions = malloc((op_max+1)*sizeof(int*));

    for(int i=0;i<op_max+1;i++){
        graphe1->matrice_exclusions[i] = malloc((op_max + 1) * sizeof(int));
        graphe1->tab_stations[i] = malloc(sizeof(t_station));
        graphe1->tab_stations[i]->val_station=i;
        graphe1->tab_stations[i]->voisins=NULL;
        graphe1->tab_stations[i]->couleur=0;
        graphe1->tab_stations[i]->marquage=0;
    }

    for(int i = 0; i < op_max + 1;i++){
        for(int j = 0; j < op_max + 1;j++){
            graphe1->matrice_exclusions[i][j] = 0;
        }
    }
...
t_graphe *creer_graphe(int op_max){
    ...
    nv_graphe->num_operation_maxi=op_max+1;
    nv_graphe->nb_stations=0;
    nv_graphe->ordre_sous_graphe=0;
    nv_graphe->sous_sommets=malloc(nv_graphe->num_operation_maxi*sizeof(int));
    nv_graphe->tab_stations = (t_station **)malloc(nv_graphe->num_operation_maxi * sizeof(t_station *));

    for(int i = 0;i < nv_graphe->num_operation_maxi;i++){
...
void graphe_afficher(t_graphe* graphe)
{
    ...
    for (int i=0; i < graphe->num_operation_maxi; i++)
    {
...
void Affichage_stations(t_graphe* un_graphe){
    ...
    for(int i = 0; i < un_graphe->nb_stations;i++){
        ...
        for(int j = 0; j < un_graphe->num_operation_maxi;j++){
...
void ColorationGlouton(t_graphe* graphe){
    ...
    int* indices = (int*)malloc(graphe->num_operation_maxi * sizeof(int));
    for(int i = 0; i < graphe->num_operation_maxi; i++){
        indices[i] = i;
    }
    ...
    for(int i = 0; i < graphe->num_operation_maxi; i++){
        for (int j = i + 1; j < graphe->num_operation_maxi; j++){
...
int nb_mini_stations_exclu(t_graphe *un_graphe){
    ...
    for(int c = 1; c < un_graphe->num_operation_maxi; c++){
        sous_graphe = creer_graphe(un_graphe->num_operation_maxi);

        un_graphe->ordre_sous_graphe = 0;

        for(int i = 0; i < un_graphe->num_operation_maxi;i++){
...
t_graphe *creation_graphe_stations_exclusion(t_graphe* un_graphe, char * file_name, char* temps_cycle){
    ...
    graphe_stations->num_operation_maxi = un_graphe->num_operation_maxi;

    graphe_stations->tab_operations_reelles = malloc(graphe_stations->num_operation_maxi * sizeof(int));
    ...
    for(int i = 0; i < graphe_stations->nb_stations;i++){
        ...
        graphe_stations->tab_stations[i]->tab_operation = malloc(un_graphe->num_operation_maxi * sizeof(t_operation*));
...
int nb_nv_stations_necessaires_exclusion_temps(t_graphe* graphe_exclusion){
    ...
    for(int i = 0; i < graphe_exclusion->nb_stations;i++){
        temps_actuel = 0;
        for(int j = 0; j < graphe_exclusion->num_operation_maxi;j++){
...
t_graphe* associer_operations_exclusion_temps(t_graphe* graphe_exclusion_temps, int nv_stations){
    ...
    int nb_nv_stations = 0;
    int* tab_stockage = malloc(graphe_exclusion_temps->num_operation_maxi * sizeof(int));
    ...
    for(int i = 0; i < graphe_exclusion_temps->num_operation_maxi; i++){
        tab_stockage[i] = -1;
    }
    ...
    for(int i = 0; i <= (graphe_exclusion_temps->nb_stations - nv_stations); i++){
        ...
        for(int v = 0; v < graphe_exclusion_temps->num_operation_maxi; v++){
            tab_stockage[v] = -1;
        }
        ...
        for(int j = 0; j < graphe_exclusion_temps->num_operation_maxi; j++){
            ...
            if(temps_actuel + graphe_exclusion_temps->tab_stations[i]->tab_operation[j]->temps_ope > temps_cycle){
                printf("\ndans station : %d\n", station_choix);
                for(int c = 0; c < graphe_exclusion_temps->num_operation_maxi;c++){
                    if(tab_stockage[c] >= 0){
