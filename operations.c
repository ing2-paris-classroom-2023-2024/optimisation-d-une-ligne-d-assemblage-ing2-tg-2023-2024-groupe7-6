...
void debutParcoursBfs(t_graphe* graphe){
    ...
    for (int i = sommetMin; i < graphe->num_operation_max; ++i) {
        printf("\nBoucle1");
        printf("\toperation %d station %d", i, graphe->tab_stations[i]->numeroStation);
    }
    ...
    for (int i = sommetMin; i < graphe->num_operation_max; ++i) {
        ...
        if (graphe->tab_perations_reelles[sommetMin] == 1) {
            if (graphe->tab_stations[i]->numeroStation == 0){
                printf("\nnoperation %d station %d", i, graphe->tab_stations[i]->numeroStation);
            } else
                printf("\noperation %d station %d", i, graphe->tab_stations[i]->numeroStation -1 );
        }
    }
}
